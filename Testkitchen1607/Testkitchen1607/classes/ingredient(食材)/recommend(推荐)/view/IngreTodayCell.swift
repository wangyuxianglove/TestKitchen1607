...
class IngreTodayCell: UITableViewCell {
        ...
        if listModel?.widget_data?.count>0{
            for i in 0..<3{
                ...
                if i*4<listModel?.widget_data?.count{
                    let imageData=listModel?.widget_data![1*4]
                ...
                if i*4+3<listModel?.widget_data?.count{
                    let desData=listModel?.widget_data![i*4+3]
                    if desData?.type=="text"{
                        
                        //获取子视图
                        let tmpView=contentView.viewWithTag(300+i)
                        if tmpView?.isKindOfClass(UILabel)==true{
                            let desLabel=tmpView as! UILabel
                            desLabel.text=desData?.content
                            print(desData?.content)
                        }
    ...
    //创建cell的方法
    class func creatRedTodayCellFor(tableView:UITableView,atIndexPath indexPath:NSIndexPath,listModel:IngreRecommendWidgetList?)->IngreTodayCell{
